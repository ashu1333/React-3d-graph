...
export default function NameGraph1() {
  ...
  const data = useMemo(() => {
    ...
    if (w1 && w2 && w3) {
      ...
      for (let i = 0; i < 5; i++) {
        let len = nodes[i].neighbors.length;
      ...
      for (let i = w1.length; i < nodes[i].length; i++) {
        let len = nodes[i].neighbors.length;
